...
class SimpleOxygenForecaster:
    ...
    def predict(self, horizon=10080, return_uncertainty=True):
        ...
        if self.last_timestamp_ is not None:
            future_times = pd.date_range(
                start=self.last_timestamp_ + pd.Timedelta(minutes=1), periods=horizon, freq="1min"
            )
        else:
            future_times = None
    ...
    def load(cls, filepath):
        ...
        forecaster = cls(
            short_window=data["short_window"], long_window=data["long_window"], alpha=data.get("alpha", 0.3)
        )
