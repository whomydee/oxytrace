...
class OxygenFeatureEngineer:
    ...
    def transform(self, df, oxygen_col="Oxygen[%sat]"):
        ...
        features["rate_change_1min"] = oxygen.diff(1).abs().fillna(0)
        features["rate_change_5min"] = oxygen.diff(5).abs().fillna(0)
        features["rate_change_60min"] = oxygen.diff(60).abs().fillna(0)
        ...
        features["range_30min"] = (
            oxygen.rolling(self.fault_window, min_periods=1).max()
        ...
        features["outlier_density_30min"] = self._compute_outlier_density(
            oxygen, features["rolling_mean_60"], features["rolling_std_60"], self.fault_window
        )
        ...
        features["pattern_deviation"] = self._compute_pattern_deviation(
            oxygen, features["rolling_mean_360"], features["rolling_std_360"]
        )
    ...
    def get_feature_names(self):
        return [
            ...
            "range_30min",
            "coeff_variation",
            "max_consecutive_same_60",
            "value_change_rate",
            "point_to_point_variance",
            "high_freq_energy",
            "outlier_density_30min",
            "autocorr_lag5",
